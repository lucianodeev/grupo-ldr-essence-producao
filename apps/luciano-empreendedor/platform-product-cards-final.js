(function () {
  ...
  function validLocale(value){const key=String(value||"").slice(0,2).toLowerCase();return COPY[key]?key:"";}
  function currentLocale(preferred){
    const forced=validLocale(preferred);if(forced)return forced;
    const active=document.querySelector(".ldr-public-lang-button.active[data-lang]");
    const fromActive=active?validLocale(active.dataset.lang):"";if(fromActive)return fromActive;
    const fromHtml=validLocale(document.documentElement.lang);if(fromHtml)return fromHtml;
    try{const saved=validLocale(localStorage.getItem(LOCALE_KEY));if(saved)return saved;}catch(e){}
    try{const query=validLocale(new URLSearchParams(window.location.search).get("lang"));if(query)return query;}catch(e){}
    return "pt";
  }
  ...
  function applyCopy(preferred){
    const copy=COPY[currentLocale(preferred)];
  ...
  function start(){
    ...
    window.addEventListener("luciano:languagechange",event=>applyCopy(event&&event.detail?event.detail.lang:""));
    ...
    let lastLocale=currentLocale();setInterval(()=>{const now=currentLocale();if(now!==lastLocale){lastLocale=now;applyCopy(now);}},250);
    document.addEventListener("click",()=>{setTimeout(()=>applyCopy(),0);setTimeout(()=>applyCopy(),100);setTimeout(()=>applyCopy(),300);},true);
    document.addEventListener("change",()=>{setTimeout(()=>applyCopy(),0);setTimeout(()=>applyCopy(),150);},true);
  }
