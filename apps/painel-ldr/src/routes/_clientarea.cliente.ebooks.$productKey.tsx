...
function EbookOffer(){
 ...
 const buy=useMutation({mutationFn:(market:"BR"|"INTL")=>checkoutFn({data:{productKey:productKey as any,market}}),onSuccess:r=>location.href=r.url});
 ...
 const product=data.products.find(p=>p.key===book.key); const entitled=Boolean(product?.entitled); const market:data["market"]="BR"|"INTL" as any;
 const selectedMarket=data.market==="BR"?"BR":"INTL";
