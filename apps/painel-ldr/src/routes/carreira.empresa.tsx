...
function CompanyArea() {
 ...
 return <main className="min-h-screen bg-slate-50 px-5 py-10"><div className="mx-auto max-w-5xl"><header className="flex flex-wrap items-center justify-between gap-4"><Link to="/carreira" className="font-bold text-[#07345b]">LDR Carreira</Link><LanguageSelect /></header><h1 className="mt-10 text-3xl font-bold text-[#07345b]">{t[0]}</h1><p className="mt-3 text-slate-600">{t[1]}</p><nav className="mt-8 grid gap-4 sm:grid-cols-2">{["/carreira/empresa/publicar", "/carreira/empresa/candidaturas", "/carreira/empresa/guia-triagem-responsavel", "/carreira/vagas"].map((to,i)=><Link key={to} to={to} className="flex min-h-14 items-center rounded-2xl border border-slate-200 bg-white p-5 font-bold text-[#07345b]">{t[i+2]}</Link>)}</nav></div></main>;
}
