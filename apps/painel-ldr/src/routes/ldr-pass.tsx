...
const plans = [
  ...
  { key: "business" as const, name: "LDR PASS BUSINESS", br: "R$ 499/mês", brAnnual: "R$ 4.990/ano", eu: "€ 99/mês", euAnnual: "€ 990/ano", desc: "Recursos premium para empresas, talentos e recrutamento.", items: ["Ferramentas empresariais premium", "Gestão de candidaturas e triagem responsável", "Banco e reaproveitamento de talentos", "Serviço humano de recrutamento permanece separado ou por créditos", "Recrutamento em grande volume não está incluído na assinatura"] },
];
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {plans.map((plan, index) => (
            <article key={plan.name} className={`rounded-[28px] border p-6 ${index === 2 ? "border-[#f4c76b]/60 bg-[#f4c76b]/[.09]" : "border-white/15 bg-white/[.06]"}`}>
              ...
              {plan.key==="business"&&<div className="mt-5 rounded-2xl border border-[#f4c76b]/30 bg-black/25 p-4 text-sm leading-6 text-white/75"><strong className="text-[#f4c76b]">Recrutamento & Seleção:</strong> o BUSINESS oferece acesso aos recursos empresariais elegíveis da plataforma, mas não inclui recrutamento humano ilimitado nem projetos de contratação em grande volume. Projetos de recrutamento em escala são avaliados e contratados separadamente, com proposta personalizada conforme volume, perfil das vagas e escopo do processo.<p className="mt-3 font-bold text-white">A assinatura BUSINESS não substitui a contratação de um projeto de Recrutamento & Seleção.</p></div>}\n              <button disabled={loading!==null} onClick={()=>checkout(plan.key)} className="mt-6 w-full rounded-xl bg-[#f4c76b] px-5 py-3 font-black text-[#1f1303] disabled:opacity-50">{loading===plan.key?"Abrindo checkout…":`Assinar ${plan.name}`}</button><p className="mt-3 text-center text-xs text-white/50">{market==="BR"?(billing==="monthly"?plan.br:plan.brAnnual):(billing==="monthly"?plan.eu:plan.euAnnual)}</p>
            </article>
          ))}
        </div>
      </section>
