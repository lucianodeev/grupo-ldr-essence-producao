    ...
    <div className="min-h-screen lg:flex" style={{ background: "var(--cream)" }}>
      <aside className="no-print sticky top-0 z-40 text-primary-foreground lg:h-screen lg:w-72 lg:shrink-0" style={{ background: "linear-gradient(160deg, var(--wine-deep), var(--wine))" }}>
        ...
        <nav className={`${menuOpen ? "block" : "hidden"} px-3 pb-4 lg:block`} aria-label={c.navLabel}>
          ...
          {!servicePortal ? <div className="mt-5 border-t border-white/20 pt-4"><p className="mb-2 text-xs font-bold uppercase tracking-wide opacity-70">{c.ecosystem}</p><a className="block rounded-lg px-3 py-2 text-sm hover:bg-white/10" href="https://ldrrhestrategia.com/" target="_blank" rel="noreferrer">Grupo LDR Essence</a><a className="block rounded-lg px-3 py-2 text-sm hover:bg-white/10" href={`https://ldrrhestrategia.com/treinamento?lang=${locale}`} target="_blank" rel="noreferrer">{c.entrepreneurs}</a></div> : null}
