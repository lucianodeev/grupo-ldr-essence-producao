...
function portalLoginDestination(portal:OAuthPortal,error?:"missing_code"|"exchange_failed",next?:string|null){const suffix=error?`?auth_error=${error}`:"?auth_complete=1";if(portal==="company"){const login=companyLoginHref(next);return `${login}${login.includes("?")?"&":"?"}${error?`auth_error=${error}`:"auth_complete=1"}`}if(portal==="employee")return`/funcionario/login${suffix}`;if(portal==="professional")return`/profissional/login${suffix}`;return error?`/cliente/login?auth_error=${error}`:"/cliente/biblioteca"}
...
export const Route=createFileRoute("/api/auth/callback")({server:{handlers:{GET:async({request})=>{
 ...
 const adminFlow=url.searchParams.get("portal")==="admin"||url.searchParams.get("admin")==="1"||requestCookies.some(({name,value})=>name==="ldr_admin_oauth"&&value==="1");
 ...
 if(!code){const destination=academicReturn&&!adminFlow&&!portalFlow&&!servicePortal?`${academicLoginHref(academicReturn)}&auth_error=missing_code`:adminFlow?"/login?auth_error=missing_code":portalFlow?portalLoginDestination(portalFlow,"missing_code",companyReturn):servicePortal?"/cliente/login?portal=services&auth_error=missing_code":"/cliente/login?auth_error=missing_code";if(adminFlow)clearTemporaryCookie(responseHeaders,"ldr_admin_oauth");if(portalFlow){clearTemporaryCookie(responseHeaders,"ldr_portal_oauth");clearTemporaryCookie(responseHeaders,COMPANY_RETURN_COOKIE);}if(academicReturn)clearTemporaryCookie(responseHeaders,ACADEMIC_RETURN_COOKIE);responseHeaders.set("location",destination);return new Response(null,{status:303,headers:responseHeaders})}
 const{error}=await supabase.auth.exchangeCodeForSession(code);
 const destination=academicReturn&&!adminFlow&&!portalFlow&&!servicePortal?(error?`${academicLoginHref(academicReturn)}&auth_error=exchange_failed`:academicReturn):error?(adminFlow?"/login?auth_error=exchange_failed":portalFlow?portalLoginDestination(portalFlow,"exchange_failed",companyReturn):servicePortal?"/cliente/login?portal=services&auth_error=exchange_failed":"/cliente/login?auth_error=exchange_failed"):(adminFlow?"/admin":portalFlow?portalLoginDestination(portalFlow,undefined,companyReturn):servicePortal?"/cliente?portal=services&v=4":"/cliente/biblioteca");
 if(adminFlow)clearTemporaryCookie(responseHeaders,"ldr_admin_oauth");if(portalFlow){clearTemporaryCookie(responseHeaders,"ldr_portal_oauth");clearTemporaryCookie(responseHeaders,COMPANY_RETURN_COOKIE);}if(academicReturn)clearTemporaryCookie(responseHeaders,ACADEMIC_RETURN_COOKIE);responseHeaders.set("location",destination);return new Response(null,{status:303,headers:responseHeaders})
}}}});
