  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        ...
        if (data.status === "pending") { setState("pending"); setTimeout(check, 1800); return; }
        setState("ready");
      } catch {
        ...
        {state === "ready" && (
          <div className="mt-4 space-y-4">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
              <strong>Pagamento confirmado.</strong><br />Seu produto ou serviço já foi associado ao acesso correto na plataforma.
            </div>
            {result?.activation_required ? (
              <>
                <p className="text-sm text-muted-foreground">{result.activation_sent ? "Enviamos para o e-mail da compra um link seguro para você definir sua senha." : "Use o mesmo e-mail da compra para ativar seu acesso."} Você será direcionado automaticamente.</p>
