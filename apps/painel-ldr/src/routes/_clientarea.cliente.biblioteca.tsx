...
function ClientLibrary(){
  ...
  const ent={ebook:owner||!!ebook?.entitled,book:owner||!!book?.entitled,training:owner||!!training?.entitled,psycho:owner||!!psycho?.entitled,mentor:owner||!!mentor?.entitled,leader:owner||!!leader?.entitled,free:owner||!!free?.entitled,french:owner||!!french?.entitled,firstAid:owner||!!firstAid?.entitled};
  ...
  const buyFormation=(kind:"psycho"|"mentor"|"leader",mut:any,color:string)=><button onClick={()=>mut.mutate(market)} className={`mt-3 w-full rounded-xl ${color} px-4 py-3 text-sm font-black text-white`}><ShoppingCart className="mr-2 inline h-4 w-4"/>Quero começar minha formação</button>;
  const postgrads=[
  ...
  return <div className="min-w-0 space-y-6 pb-8">
    ...
    <section className="rounded-[28px] border bg-[#fffdf8] p-5 shadow-sm sm:p-7">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <article className="rounded-2xl border border-[#d9c6ea] bg-[#fbf7ff] p-5"><span className="text-[10px] font-black uppercase tracking-[.12em] text-[#5b2b86]">Psicanálise</span><h3 className="mt-2 font-serif text-xl text-[#32155c]">Formação Online em Psicanálise</h3><p className="mt-2 text-sm text-muted-foreground">12 módulos · 180 unidades · percurso formativo completo.</p>{ent.psycho?<a href="/cliente/treinamentos/psicanalise" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#5b2b86] px-4 py-3 text-sm font-black text-white">Acessar formação</a>:buyFormation("psycho",psychoCheckout,"bg-[#5b2b86]")}</article>
