...
function ClientLibrary() {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <section className="grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label={copy.title}>
      ...
      <article className="s8-card flex min-h-[360px] min-w-0 flex-col overflow-hidden" data-product="do-mamao-ao-negocio" style={{borderColor:"#c9a63a"}}>
        <div className="flex items-start justify-between gap-3"><div className="shrink-0 rounded-xl border p-3" style={{borderColor:"#c9a63a",background:"#0b2341",color:"#f0cf78"}}><GraduationCap className="h-6 w-6" /></div><span className={`max-w-[65%] rounded-full px-3 py-1 text-center text-xs font-bold ${trainingOffer?.entitled ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}`}>{trainingOffer?.entitled ? copy.accessState : copy.buyState}</span></div>
        <p className="mt-5 break-words text-xs font-bold uppercase tracking-[.14em]" style={{color:"#8a6816"}}>{copy.trainingMeta}</p><h2 className="mt-1 break-words font-serif text-2xl">Do Mamão ao Negócio</h2><p className="mt-2 break-words text-sm leading-6 text-muted-foreground">{copy.trainingDesc}</p>
