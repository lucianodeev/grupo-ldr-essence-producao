...
const TXT={
  pt:{title:"Minha Biblioteca",intro:"Aprenda no seu ritmo. Continue de onde parou ou descubra algo novo por aqui.",orders:"Ver pedidos e pagamentos",access:"Acesso liberado",buy:"Comprar",start:"Começar gratuitamente",open:"Acessar curso",comments:"Conversas sobre este conteúdo",french:"Francês A1",aid:"Primeiros Socorros",freud:"Clínica Psicanalítica: Freud",freeHours:"120h · GRATUITO"},
  en:{title:"My Library",intro:"Your private space for reading, training and content.",orders:"View orders and payments",access:"Access granted",buy:"Buy",start:"Start for free",open:"Open course",comments:"Comments",french:"French A1",aid:"First Aid",freud:"Psychoanalytic Clinic: Freud",freeHours:"120h · FREE"},
  fr:{title:"Ma Bibliothèque",intro:"Votre espace privé de lecture, formation et contenus.",orders:"Voir les commandes et paiements",access:"Accès autorisé",buy:"Acheter",start:"Commencer gratuitement",open:"Ouvrir le cours",comments:"Commentaires",french:"Français A1",aid:"Premiers secours",freud:"Clinique psychanalytique : Freud",freeHours:"120 h · GRATUIT"},
  es:{title:"Mi Biblioteca",intro:"Tu espacio privado de lectura, formación y contenidos.",orders:"Ver pedidos y pagos",access:"Acceso liberado",buy:"Comprar",start:"Comenzar gratis",open:"Abrir curso",comments:"Comentarios",french:"Francés A1",aid:"Primeros Auxilios",freud:"Clínica Psicoanalítica: Freud",freeHours:"120 h · GRATIS"}
} as const;

const SUB_TXT={
  pt:{eyebrow:"ACESSO TOTAL",title:"Assine a Biblioteca LDR",desc:"Uma única assinatura para acessar os conteúdos digitais marcados como incluídos enquanto estiver ativa. Compras avulsas continuam com acesso vitalício.",included:"Cursos, formações, treinamentos, eBooks e livros marcados como incluídos",subscribe:"ASSINAR AGORA",active:"Assinatura ativa",cancel:"Cancelar ao fim do período",keep:"Manter assinatura",ending:"Cancelamento agendado"},
  en:{eyebrow:"ALL ACCESS",title:"Subscribe to Biblioteca LDR",desc:"One subscription gives access to the digital content marked as included while active. One-time purchases keep lifetime access.",included:"Courses, training, eBooks and books marked as included",subscribe:"SUBSCRIBE NOW",active:"Active subscription",cancel:"Cancel at period end",keep:"Keep subscription",ending:"Cancellation scheduled"},
  fr:{eyebrow:"ACCÈS TOTAL",title:"Abonnez-vous à Biblioteca LDR",desc:"Un seul abonnement donne accès aux contenus numériques indiqués comme inclus tant qu'il reste actif. Les achats à l'unité restent accessibles à vie.",included:"Cours, formations, eBooks et livres indiqués comme inclus",subscribe:"S'ABONNER",active:"Abonnement actif",cancel:"Annuler à la fin de la période",keep:"Maintenir l'abonnement",ending:"Annulation programmée"},
  es:{eyebrow:"ACCESO TOTAL",title:"Suscríbete a Biblioteca LDR",desc:"Una sola suscripción da acceso a los contenidos digitales marcados como incluidos mientras esté activa. Las compras individuales mantienen acceso de por vida.",included:"Cursos, formaciones, eBooks y libros marcados como incluidos",subscribe:"SUSCRIBIRME",active:"Suscripción activa",cancel:"Cancelar al final del período",keep:"Mantener suscripción",ending:"Cancelación programada"}
} as const;
...
function ClientLibrary(){
  ...
  const subscriptionCheckout=useMutation({mutationFn:(x:{market:"BR"|"INTL";billingCycle:"monthly"|"annual"})=>subscriptionCheckoutFn({data:x}),onSuccess:r=>location.href=r.url});
  ...
  return <div className="min-w-0 space-y-6 pb-8">
    ...
    <details id="catalogo-ldr" open className="scroll-mt-24 rounded-[24px] border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid grid-cols-4 gap-1.5 sm:grid-cols-10 sm:gap-3">
        {card("ebook",ct.ebooks,"bg-[#5b0824]","book",accessLabels.included)}
        {card("book",ct.books,"bg-[#7A3E12]","book",accessLabels.included)}
        ...
        <a href="/cliente/biblioteca/publicacoes/revista-psicanalise-no-mundo" className="min-w-0 rounded-2xl bg-[#0b2341] px-1 py-4 text-center !text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" aria-label="Abrir todos os jornais e revistas"><div className="mx-auto text-lg leading-none">📰</div><p className="mt-2 text-[8px] font-black leading-tight !text-white sm:text-[10px]">{ct.editorial}</p><p className="mt-1 text-[7px] !text-white/80">{ct.editorialSub}</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase text-white">{accessLabels.editorial}</span></a>
        {card("training",ct.business,"bg-[#d6ad63]","grad",accessLabels.included)}
        {card("psycho",ct.psycho,"bg-[#5b2b86]","grad",accessLabels.single)}
        {card("brief",ct.brief,"bg-[#17645e]","grad",accessLabels.included)}
        {card("massage",ct.massage,"bg-[#0F5E7A]","grad",accessLabels.included)}
        <a href="/clinica-social" className="min-w-0 rounded-2xl bg-[#2F6F4E] px-1 py-4 text-center text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><span className="mx-auto block text-lg">🤝</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">Clínica Social</p></a>
        ...
        <a href="/cliente/orientacao-profissional" className="min-w-0 rounded-2xl bg-[#4B3F8F] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">💼</span><p className="mt-2 text-[8px] font-black leading-none sm:text-[10px]">{ct.writtenCareer}</p></a>
        {card("business24",ct.business24,"bg-[#c85a24]","grad",accessLabels.included)}
        {card("mentor",ct.mentorship,"bg-[#0b5cab]","grad",accessLabels.included)}
        {card("leader",ct.leadership,"bg-[#0f5132]","grad",accessLabels.included)}
        {card("hr",ct.hr,"bg-[#047857]","grad",accessLabels.included)}
        <a href="/cliente/cursos/organizar-carreira" className="min-w-0 rounded-2xl bg-[#b85c2e] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">💼</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">Carreira</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase">{accessLabels.free}</span></a>
        <a href="/cliente/cursos/frances-negocios-a1" className="min-w-0 rounded-2xl bg-[#123f73] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">🇫🇷</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">{t.french}</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase">{accessLabels.free}</span></a>
        <a href="/cliente/cursos/primeiros-socorros" className="min-w-0 rounded-2xl bg-[#b4232a] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">⛑️</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">{t.aid}</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase">{accessLabels.free}</span></a>
        <a href="/cliente/cursos/academy/clinica-psicanalitica-sigmund-freud" className="min-w-0 rounded-2xl bg-[#5b2b86] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">🛋️</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">{t.freud}</p><span className="mt-2 block text-[8px] font-bold text-white/75">{t.freeHours}</span></a>
        ...
        {card("ai","IA","bg-[#143d59]","grad",accessLabels.single)}
        <a href="/cliente/treinamentos/psicanalise-internacional" className="min-w-0 rounded-2xl bg-[#24476f] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">🌍</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">Psicanálise Internacional</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase">{accessLabels.single}</span></a>
        <a href="/cliente/psicanalista-alta-performance" className="min-w-0 rounded-2xl bg-[#8a6a2d] px-1 py-4 text-center text-white shadow-sm"><span className="mx-auto block text-lg">📈</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">Alta Performance</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase">{accessLabels.single}</span></a>
        {card("film","Filme","bg-[#35101e]","film")}
        {PROFESSIONAL_FORMATIONS.map((f,i)=>{const ft=pfText(f,locale);const included=includedProfessionalSlugs.has(f.slug);const colors=["#6F4E37","#3E6B89","#8C4A5B","#6A5A91","#2E7D6E","#9A6B2F","#4F6B3C","#7B4F8C","#A14F36","#356B72","#76543A","#5D6F8A","#884C63","#3E745B","#8B6A3E","#526B88","#6D3FA0","#2F6F73","#A65A3A","#7A3651","#315F86","#B1842F"];return <a key={f.slug} href={f.learnerPath} className="min-w-0 rounded-2xl px-1 py-4 text-center text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" style={{backgroundColor:colors[i%colors.length]}}><span className="mx-auto block text-lg">{f.icon}</span><p className="mt-2 text-[8px] font-black leading-tight sm:text-[10px]">{ft.short}</p><span className="mt-2 inline-flex rounded-full bg-white/15 px-1.5 py-0.5 text-[6px] font-black uppercase tracking-wide text-white sm:text-[7px]">{included?accessLabels.included:accessLabels.single}</span></a>})}
      </div>
      {active?<div className="mt-4 rounded-2xl border bg-white p-5 shadow-inner">
        ...
        </div>:active==="ai"?<div className="rounded-2xl border border-[#9ec1d6] bg-gradient-to-br from-[#f5fbff] to-[#eef5f8] p-5"><span className="rounded-full bg-[#143d59] px-3 py-1 text-[10px] font-black uppercase tracking-[.14em] text-white">600H · 100% ONLINE · SEM AULAS AO VIVO</span><h3 className="mt-4 font-serif text-2xl text-[#143d59]">🤖 Formação em Inteligência Artificial Aplicada aos Negócios e à Carreira</h3><p className="mt-3 text-base font-black text-[#143d59]">15 módulos · 225 aulas · 5 projetos avaliativos + Projeto Final.</p><p className="mt-2 text-xs font-bold text-[#143d59]">Compra avulsa · não incluído na assinatura Biblioteca LDR · acesso vitalício após compra.</p><p className="mt-2 text-sm leading-6 text-muted-foreground">IA, conteúdo, marketing digital, vendas, gestão de cursos, programação moderna, sites e sistemas, automações, agentes, RH, produtividade, empreendedorismo e carreira.</p>{ent.ai?<a href="/cliente/treinamentos/ia-negocios-carreira" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#143d59] px-4 py-3 text-sm font-black text-white">ACESSAR FORMAÇÃO</a>:<button disabled={aiCheckout.isPending} onClick={()=>aiCheckout.mutate(market)} className="mt-4 w-full rounded-xl bg-[#143d59] px-4 py-3 text-sm font-black text-white">{market==="BR"?money(ai?.priceBrlCents??29999,"BRL",locale):money(ai?.priceEurCents??4990,"EUR",locale)} · ACESSO VITALÍCIO</button>}</div>:active==="ebook"&&ebook?<div><h3 className="font-serif text-xl">A Coragem de Começar</h3>{ent.ebook?<a href="/cliente/biblioteca/ebook_coragem_comecar" className="mt-4 inline-flex w-full justify-center rounded-xl bg-primary px-4 py-3 text-sm font-bold text-white">Acessar eBook</a>:<button onClick={()=>checkout.mutate({productKey:"ebook_coragem_comecar",market})} className="mt-4 w-full rounded-xl bg-primary px-4 py-3 text-white">{t.buy}</button>}</div>:active==="book"&&book?<div><h3 className="font-serif text-xl">O Menino que Vendia Mamão</h3>{ent.book?<a href="/cliente/biblioteca/livro_menino_mamao" className="mt-4 inline-flex w-full justify-center rounded-xl bg-primary px-4 py-3 text-sm font-bold text-white">Acessar livro</a>:<button onClick={()=>checkout.mutate({productKey:"livro_menino_mamao",market})} className="mt-4 w-full rounded-xl bg-primary px-4 py-3 text-white">{t.buy}</button>}</div>:active==="training"?<div><h3 className="font-serif text-xl">Do Mamão ao Negócio</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">Formação para empreendedores, atividades práticas e percurso estruturado.</p>{ent.training?<a href="/cliente/treinamentos/do-mamao-ao-negocio" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#d6ad63] px-4 py-3 font-bold text-[#281605]">Acessar treinamento</a>:<button onClick={()=>trainingCheckout.mutate(market)} className="mt-4 w-full rounded-xl bg-[#d6ad63] px-4 py-3 font-bold text-[#281605]">{market==="BR"?money(training?.priceBrlCents??29999,"BRL",locale):money(training?.priceEurCents??4990,"EUR",locale)}</button>}</div>:active==="psycho"?<div><h3 className="font-serif text-xl text-[#32155c]">Formação Online em Psicanálise</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">14 módulos · 220 unidades · percurso formativo completo.</p>{ent.psycho?<a href="/cliente/treinamentos/psicanalise" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#5b2b86] px-4 py-3 font-black text-white">Acessar formação</a>:buyFormation("psycho",psychoCheckout,"bg-[#5b2b86]")}</div>:active==="business24"?<div><h3 className="font-serif text-xl text-[#8a3b17]">Formação Negócio em 24 Horas</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">9 módulos · 90 aulas · 360 horas · 100% online · sem encontros ao vivo · projeto final avaliado.</p>{ent.business24?<a href="/cliente/treinamentos/negocio-24-horas" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#c85a24] px-4 py-3 font-black text-white">Acessar formação</a>:<button onClick={()=>business24Checkout.mutate(market)} className="mt-4 w-full rounded-xl bg-[#c85a24] px-4 py-3 font-black text-white">Quero começar · {market==="BR"?money(business24?.priceBrlCents??29999,"BRL",locale):money(business24?.priceEurCents??4990,"EUR",locale)}</button>}</div>:active==="brief"?<div><h3 className="font-serif text-xl text-[#103d3a]">Formação em Terapia Breve Psicanalítica</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">15 módulos · 300 aulas · 1.200 horas · 6 encontros ao vivo.</p>{ent.brief?<a href="/cliente/treinamentos/terapia-breve-psicanalitica" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#17645e] px-4 py-3 font-black text-white">Acessar formação</a>:buyFormation("brief",briefCheckout,"bg-[#17645e]")}</div>:active==="massage"?<div><h3 className="font-serif text-xl text-[#083B50]">Formação Completa em Massoterapia</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">15 módulos · 300 aulas · 1.200 horas · 6 a 12 meses · 100% online · sem encontros ao vivo.</p>{ent.massage?<a href="/cliente/treinamentos/massoterapia" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#0F5E7A] px-4 py-3 font-black text-white">Acessar formação</a>:buyFormation("massage",massageCheckout,"bg-[#0F5E7A]")}</div>:active==="mentor"?<div><h3 className="font-serif text-xl text-[#0f2f57]">Formação em Mentoria Profissional e de Carreira</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">3 meses · 90 aulas · 1 aula/dia · ≈40 min/dia · acesso digital vitalício.</p>{ent.mentor?<a href="/cliente/treinamentos/mentoria-carreira" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#0b5cab] px-4 py-3 font-black text-white">Acessar formação</a>:buyFormation("mentor",mentorCheckout,"bg-[#0b5cab]")}</div>:active==="leader"?<div><h3 className="font-serif text-xl text-[#0f5132]">Formação em Liderança e Gestão de Pessoas</h3><p className="mt-2 text-xs font-bold text-[#0b2341]">✓ Incluído na assinatura Biblioteca LDR · compra avulsa mantém acesso vitalício.</p><p className="mt-2 text-sm text-muted-foreground">3 meses · 90 aulas · 1 aula/dia · ≈40 min/dia · acesso digital vitalício.</p>{ent.leader?<a href="/cliente/treinamentos/lideranca-gestao" className="mt-4 inline-flex w-full justify-center rounded-xl bg-[#0f5132] px-4 py-3 font-black text-white">Acessar formação</a>:buyFormation("leader",leaderCheckout,"bg-[#0f5132]")}</div>:<div><h3 className="font-serif text-xl">O Menino que Vendia Mamão</h3><p className="mt-2 text-sm text-muted-foreground">Filme em produção. Ainda não há conteúdo liberado.</p></div>}
      </div>:null}
      </div>
    </details>
    ...
    <section className="rounded-[28px] border border-[#d6ad63]/40 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="w-full shrink-0 rounded-2xl bg-[#071426] p-4 text-white sm:w-[280px]">
          ...
          {subscriptionData?.active?<div className="mt-3"><p className="text-xs text-white/75">{subscriptionData?.subscription?.cancel_at_period_end?SUB_TXT[locale].ending:"Renovação automática"}</p><button disabled={subscriptionCancel.isPending} onClick={()=>subscriptionCancel.mutate(!subscriptionData?.subscription?.cancel_at_period_end)} className="mt-3 w-full rounded-xl bg-white px-3 py-2 text-xs font-black text-[#071426]">{subscriptionData?.subscription?.cancel_at_period_end?SUB_TXT[locale].keep:SUB_TXT[locale].cancel}</button></div>:<button disabled={subscriptionCheckout.isPending} onClick={()=>subscriptionCheckout.mutate({market,billingCycle:"monthly"})} className="mt-3 w-full rounded-xl bg-[#d6ad63] px-4 py-3 text-sm font-black text-[#281605]">{SUB_TXT[locale].subscribe}</button>}
          {!subscriptionData?.active&&market==="BR"?<div className="mt-3 border-t border-white/15 pt-3"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#ffd84d]">MELHOR VALOR · PLANO ANUAL</p><p className="mt-1 text-2xl font-black text-white">{money(subscriptionData?.annualPriceBrlCents??39900,"BRL",locale)}<span className="text-xs font-medium text-white/65"> / ano</span></p><p className="mt-1 text-xs text-white/70">Equivale a R$ 33,25/mês · cobrança anual</p><button disabled={subscriptionCheckout.isPending} onClick={()=>subscriptionCheckout.mutate({market:"BR",billingCycle:"annual"})} className="mt-3 w-full rounded-xl bg-white px-4 py-3 text-sm font-black text-[#071426]">QUERO ACESSO ANUAL</button></div>:null}
        </div>
      </div>
    </section>

    <section className="mb-6"><a href="/cliente/rede-academica" className="group block rounded-[24px] border border-[#d8cba9] bg-gradient-to-r from-[#071426] via-[#102a4a] to-[#193b62] p-5 text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-black tracking-[.16em] text-[#e7c979]">REDE ACADÊMICA</p><h2 className="mt-2 font-serif text-2xl">Seu diário, suas comunidades e pessoas que compartilham seus interesses.</h2><p className="mt-2 text-sm text-slate-200">{({pt:"ACESSO GRATUITO",en:"FREE ACCESS",fr:"ACCÈS GRATUIT",es:"ACCESO GRATUITO"})[locale]}</p></div><span className="inline-flex shrink-0 rounded-xl bg-white px-4 py-3 text-xs font-black text-[#071426]">ENTRAR NA REDE →</span></div></a></section>
...
<section className="hidden rounded-[28px] border border-[#d6ad63]/35 bg-gradient-to-br from-[#071426] via-[#0b2341] to-[#102d50] p-5 text-white shadow-lg sm:p-7">
    ...
    <section className="hidden rounded-[28px] border bg-[#fffdf8] p-5 shadow-sm sm:p-7">
      <div><span className="rounded-full bg-[#0b2341] px-3 py-1 text-[10px] font-black uppercase tracking-[.15em] text-white">{ac.professionalSection}</span><h2 className="mt-4 font-serif text-3xl text-[#0b2341]">{ac.professionalTitle}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{ac.professionalIntro}</p></div>
