...
function CompanySubscriptionPage() {
  ...
  const monthlyCredits = subscription?.plan_code === "essential" ? 4 : subscription?.plan_code === "pro" ? 12 : Number(subscription?.extra_credits ?? 0);
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <section className="mt-8"><div className="mb-4"><p className="text-xs font-black uppercase tracking-[.16em] text-primary">{copy.choose}</p><h2 className="mt-1 font-serif text-3xl">{copy.choose}</h2></div><div className="grid gap-5 lg:grid-cols-3">
        <PlanCard title={copy.essential} range={`1–10 ${copy.employees}`} price={money(pricing.essentials.monthlyCents, pricing.currency, locale)} month={copy.month} bullets={[`4 ${copy.creditsIncluded}`,copy.dashboard,copy.employeeManagement,copy.monthlyRenewal]} disabled={Boolean(live)||busy} button={subscription?.status === "canceled" ? copy.renew : copy.subscribe} onClick={()=>start("essential", Math.min(Math.max(employees,1),10))}/>
        <PlanCard title={copy.pro} range={`11–50 ${copy.employees}`} price={money(pricing.pro.monthlyCents, pricing.currency, locale)} month={copy.month} bullets={[`12 ${copy.creditsIncluded}`,copy.dashboard,copy.employeeManagement,copy.monthlyRenewal]} featured disabled={Boolean(live)||busy} button={subscription?.status === "canceled" ? copy.renew : copy.subscribe} onClick={()=>start("pro", Math.min(Math.max(employees,11),50))}/>
