...
import { ArrowUpRight, BriefcaseBusiness, Building2, HeartHandshake, Sparkles, UserRound, UsersRound } from "lucide-react";
...
function Home() {
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <section className="relative mt-4 overflow-hidden rounded-[2rem] border bg-card p-7 shadow-xl shadow-primary/5 sm:p-9"><div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-secondary/15 blur-3xl"/><div className="relative grid gap-7 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-black uppercase tracking-[.16em] text-primary"><HeartHandshake className="h-4 w-4"/>Benefícios para empresas</div><h2 className="mt-4 max-w-3xl font-serif text-3xl leading-tight sm:text-4xl">Ofereça cuidado, desenvolvimento e bem-estar como benefício para seus funcionários.</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground sm:text-base">Planos empresariais de 1 a 10 funcionários, 11 a 50 e configuração personalizada a partir de 51 funcionários. A empresa cadastra a equipe, libera benefícios e acompanha somente as informações administrativas necessárias.</p><div className="mt-5 grid gap-2 text-sm sm:grid-cols-3"><span className="rounded-xl border bg-background px-4 py-3 font-bold">Psicanálise e atendimento individual</span><span className="rounded-xl border bg-background px-4 py-3 font-bold">Carreira, orientação e mentoria</span><span className="rounded-xl border bg-background px-4 py-3 font-bold">Bem-estar e ações corporativas</span></div></div><div className="flex flex-col gap-3 sm:flex-row lg:flex-col"><Link to="/planos-empresas" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-4 text-sm font-black text-primary-foreground shadow-lg transition hover:-translate-y-0.5">Ver planos para empresas <ArrowUpRight className="h-4 w-4"/></Link><Link to="/empresa/login" className="inline-flex items-center justify-center rounded-2xl border px-6 py-4 text-sm font-black text-primary transition hover:bg-primary/5">Acessar painel da empresa</Link></div></div></section>
