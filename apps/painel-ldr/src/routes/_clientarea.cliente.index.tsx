...
function ClientHome() {
  ...
  return <div className="space-y-6">
    ...
    <section><h2 className="mb-3 font-serif text-xl">Acessos rápidos</h2><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Link data-card="ecosystem-academy-ldr" to="/ecossistema" className="rounded-2xl border border-primary/25 bg-primary/5 p-5 text-foreground transition hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-md"><BookOpen className="h-6 w-6 text-primary"/><h3 className="mt-3 font-bold">Ecossistema LDR Academy</h3><p className="mt-1 text-sm text-muted-foreground">Acesse cursos, Biblioteca, Rede Acadêmica, carreira, vagas gratuitas e projetos do ecossistema.</p></Link>{QUICK.map(item=>{const Icon=item.icon;return <Link key={item.title} to={item.to} className={`rounded-2xl border p-5 transition hover:-translate-y-0.5 hover:shadow-md ${"featured" in item && item.featured ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"}`}><Icon className="h-6 w-6"/><h3 className="mt-3 font-bold">{item.title}</h3><p className={`mt-1 text-sm ${"featured" in item && item.featured ? "opacity-85" : "text-muted-foreground"}`}>{item.text}</p></Link>})}</div></section>
