...
export function SalesCatalog360(){
  ...
  const cards:Card[]=[
    ...
    {key:"books",label:t.books,desc:t.booksText,meta:locale==="en"?"eBooks · Books":locale==="fr"?"eBooks · Livres":locale==="es"?"eBooks · Libros":"eBooks · Livros",href:"/cliente/biblioteca",icon:BookOpen,kind:"books"},
  ...
  return <section id="catalogo-360" className="border-y border-[#d6ad63]/20 bg-[#fffdf8] py-8 sm:py-10">
    <div className="mx-auto max-w-7xl px-4 sm:px-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{filtered.map(c=>{const Icon=c.icon; const inner=<><div className="flex items-start justify-between gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#071426] text-white"><Icon className="h-5 w-5"/></span><span className="max-w-[70%] text-right text-xs font-black leading-5 text-[#8a5d17]">{c.meta}</span></div><h3 className="mt-5 text-xl font-black text-[#071426]">{c.label}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{c.desc}</p></>; return c.href.startsWith('#')?<a key={c.key} href={c.href} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-md">{inner}</a>:<Link key={c.key} to={c.href as any} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-md">{inner}</Link>})}</div>
