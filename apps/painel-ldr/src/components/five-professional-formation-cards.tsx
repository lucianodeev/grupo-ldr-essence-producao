...
export function FiveProfessionalFormationCards({compact=false}:{compact?:boolean}){
  ...
  return <section className={compact?"mt-5":"mx-auto max-w-7xl px-4 py-14 sm:px-6"}>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{PROFESSIONAL_FORMATIONS.map(f=>{const t=pfText(f,locale);return <a key={f.slug} href={compact?f.learnerPath:f.publicPath} className="rounded-[24px] border border-[#d6ad63]/30 bg-white p-5 text-[#071426] shadow-sm transition hover:-translate-y-1 hover:shadow-lg"><div className="flex items-start justify-between gap-3"><div className="text-3xl">{f.icon}</div>{t.category?<span className="rounded-full bg-[#f7f3e9] px-2 py-1 text-[9px] font-black text-[#7a5a22]">{t.category}</span>:null}</div><span className="mt-4 inline-block rounded-full bg-[#071426] px-3 py-1 text-[10px] font-black text-white">{u.online}</span><h3 className="mt-3 font-serif text-xl font-bold">{t.name}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{t.description}</p><div className="mt-4 flex flex-wrap gap-2 text-[10px] font-black"><span className="rounded-full bg-[#f7f3e9] px-2 py-1">{u.hours}</span><span className="rounded-full bg-[#f7f3e9] px-2 py-1">{f.lessons} {u.lessons}</span><span className="rounded-full bg-[#f7f3e9] px-2 py-1">{f.modulesCount} {u.modules}</span><span className="rounded-full bg-[#f7f3e9] px-2 py-1">{u.project}</span></div><p className="mt-4 font-black">R$ 299,99 · € 49,90</p><p className="mt-1 text-[10px] text-slate-500">Pagamento seguro via Stripe · outra forma de pagamento: entre em contato.</p></a>})}</div>
  </section>;
}
