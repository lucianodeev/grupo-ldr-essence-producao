...
export async function getOrganizationDashboard(userId: string) {
  ...
  const [{ data: members }, { data: services }, { data: purchases }, { data: benefits }] = await Promise.all([
    db.from("organization_members").select("id,full_name,email,department,employee_code,portal_active,auth_user_id,created_at").eq("organization_id", org.id).order("full_name"),
    ...
    db.from("organization_benefit_allocations").select("id,member_id,catalog_key,credits_granted,credits_used,status,created_at,purchase_id").eq("organization_id", org.id),
  ]);
...
export async function getEmployeeContext(userId: string, email: string | null) {
  ...
  let { data: member } = await db.from("organization_members").select("id,organization_id,full_name,email,department,employee_code,portal_active,auth_user_id").eq("auth_user_id", userId).maybeSingle();
  if (!member) {
    const result = await db.from("organization_members").select("id,organization_id,full_name,email,department,employee_code,portal_active,auth_user_id").ilike("email", mail).is("auth_user_id", null).limit(2);
    if ((result.data ?? []).length === 1) {
      const row = result.data[0];
      const bound = await db.from("organization_members").update({ auth_user_id: userId }).eq("id", row.id).is("auth_user_id", null).select("id,organization_id,full_name,email,department,employee_code,portal_active,auth_user_id").maybeSingle();
  ...
  const [{ data: organization }, { data: benefits }] = await Promise.all([
    ...
    db.from("organization_benefit_allocations").select("id,catalog_key,credits_granted,credits_used,status,created_at,purchase_id").eq("member_id", member.id).neq("status", "revoked").order("created_at", { ascending: false }),
  ]);
...
export async function requestEmployeeBenefit(userId: string, email: string | null, benefitId: string) {
  ...
  const { data, error } = await db.from("organization_benefit_allocations").update({ status: "requested", requested_at: new Date().toISOString() }).eq("id", benefitId).eq("member_id", ctx.member.id).eq("status", "assigned").select("id").maybeSingle();
