...
async function requireMaster(supabase:any,userId:string) {
 const {data:role}=await supabase.from("user_roles").select("role").eq("user_id",userId).eq("role","superadmin").maybeSingle();
 if(!role) throw new Error("Forbidden");
 return supabase;
}
export const listEcosystemContacts=createServerFn({method:"GET"}).middleware([requireSupabaseAuth]).handler(async({context})=>{
 const db=await requireMaster(context.supabase,context.userId); const {data,error}=await (db as any).from("ecosystem_contacts").select("*").order("created_at",{ascending:false}).limit(200); if(error) throw error; return data??[];
});
export const updateEcosystemContact=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((d:{id:string;status:string;priority:string})=>d).handler(async({data,context})=>{
...
export const getEcosystemAttachmentUrl=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((d:{id:string})=>d).handler(async({data,context})=>{
 const db=await requireMaster(context.supabase,context.userId); const {data:a,error}=await (db as any).from("ecosystem_contact_attachments").select("storage_path").eq("id",data.id).single();if(error)throw error;
