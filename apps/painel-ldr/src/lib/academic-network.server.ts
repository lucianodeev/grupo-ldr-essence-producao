...
export async function createAcademicPost(userId:string,input:{body:string;postType:PostType;anonymous:boolean;communityId?:string|null}){
  await requirePremium(userId); const body=clean(input.body,12000); if(!body)fail("Escreva algo antes de publicar."); if(!POST_TYPES.has(input.postType))fail("Tipo inválido.");
...
export async function moderateAcademic(userId:string,input:{kind:"post"|"comment"|"report";id:string;action:"hide"|"restore"|"pin"|"unpin"|"resolve"|"dismiss"}){ await requireAdmin(userId); if(input.kind==="post"){const patch:any={updated_at:new Date().toISOString()}; if(input.action==="hide")patch.status="hidden"; else if(input.action==="restore")patch.status="active"; else if(input.action==="pin")patch.is_pinned=true; else if(input.action==="unpin")patch.is_pinned=false; else fail("Ação inválida."); await db.from("academic_posts").update(patch).eq("id",input.id);} else if(input.kind==="comment"){if(!["hide","restore"].includes(input.action))fail("Ação inválida."); await db.from("academic_comments").update({status:input.action==="hide"?"hidden":"active",updated_at:new Date().toISOString()}).eq("id",input.id);} else {if(!["resolve","dismiss"].includes(input.action))fail("Ação inválida."); await db.from("academic_reports").update({status:input.action==="resolve"?"resolved":"dismissed",reviewed_by:userId,reviewed_at:new Date().toISOString()}).eq("id",input.id);} return {ok:true}; }
