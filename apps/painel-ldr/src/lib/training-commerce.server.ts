...
async function trainingRow() {
  const { data, error } = await db
    ...
    .select("id,slug,title,description,status,minimum_days,live_sessions_included,projects_included,lifetime_access,extra_project_review_brl_minor,extra_project_review_eur_minor")
    .eq("slug", TRAINING_SLUG)
    .maybeSingle();
...
async function ensureOwnerEnrollment(customerId: string, trainingId: string, email: string | null, userId: string) {
  ...
  const { error } = await db
    .from("training_enrollments")
    .upsert({ training_id: trainingId, customer_id: customerId, active: true }, { onConflict: "training_id,customer_id" });
...
async function syncPaidProjectReviewCredits(customerId: string, trainingId: string) {
  ...
  for (const order of (orders ?? []).filter(projectReviewMatches)) {
    await db.from("training_project_review_credits").upsert({
      training_id: trainingId,
      customer_id: customerId,
      source: "paid",
      status: "available",
      order_id: order.id,
      amount_minor: order.amount_cents ?? null,
...
export async function createDoMamaoTrainingCheckout(userId: string, email: string | null, market: Market) {
  ...
  const { data: order, error: orderError } = await db.from("orders").insert({
    ...
    description: "Treinamento de 3 meses / 300 horas com acesso vitalício ao conteúdo",
    quantity: 1,
...
export async function createDoMamaoProjectReviewCheckout(userId: string, email: string | null, market: Market) {
  ...
  const { data: order, error: orderError } = await db.from("orders").insert({
    ...
    contact_phone: customer.phone,
    service_type: "produto_digital",
    title: PROJECT_REVIEW_TITLE,
    description: "Crédito para uma nova entrega e avaliação individual de projeto",
    quantity: 1,
    amount_cents: amountCents,
    currency,
    payment_status: "pendente",
    status: "novo",
    priority: "media",
    catalog_key: PROJECT_REVIEW_KEY,
    metadata: { product_key: PROJECT_REVIEW_KEY, training_slug: TRAINING_SLUG, market, auth_user_id: userId },
  }).select("id").single();
  ...
  params.set("line_items[0][price_data][product_data][name]", PROJECT_REVIEW_TITLE);
  params.set("line_items[0][quantity]", "1");
  params.set("success_url", `${appOrigin()}/cliente/treinamentos/do-mamao-ao-negocio?project_review=success&session_id={CHECKOUT_SESSION_ID}`);
  params.set("cancel_url", `${appOrigin()}/cliente/treinamentos/do-mamao-ao-negocio?project_review=cancel`);
  params.set("client_reference_id", userId);
  params.set("metadata[order_id]", order.id);
  params.set("metadata[product_key]", PROJECT_REVIEW_KEY);
  params.set("metadata[training_slug]", TRAINING_SLUG);
  params.set("metadata[user_id]", userId);
  params.set("metadata[market]", market);
  params.set("payment_intent_data[metadata][order_id]", order.id);
  params.set("payment_intent_data[metadata][product_key]", PROJECT_REVIEW_KEY);
  if (customer.email) params.set("customer_email", customer.email);
  let response: Response;
  try {
    response = await fetch("https://api.stripe.com/v1/checkout/sessions", { method: "POST", headers: { Authorization: `Bearer ${secret}`, "Content-Type": "application/x-www-form-urlencoded" }, body: params });
...
export async function submitDoMamaoProject(userId: string, email: string | null, input: { title: string; projectUrl?: string | null; projectText?: string | null }) {
  ...
  const eligibleAt = new Date(new Date(currentEnrollment.enrolled_at).getTime() + Number(training.minimum_days ?? DEFAULT_MINIMUM_DAYS) * 86400000);
  if (Date.now() < eligibleAt.getTime()) fail("A entrega oficial do projeto é liberada ao concluir os 3 meses da formação.");
  await syncPaidProjectReviewCredits(customer.id, training.id);
  const { data: credit } = await db.from("training_project_review_credits").select("id").eq("training_id", training.id).eq("customer_id", customer.id).eq("status", "available").order("created_at", { ascending: true }).limit(1).maybeSingle();
  if (!credit) fail("Você não possui crédito disponível para uma nova avaliação de projeto.");
  const { count } = await db.from("training_project_submissions").select("id", { count: "exact", head: true }).eq("enrollment_id", currentEnrollment.id);
  const submissionNumber = Number(count ?? 0) + 1;
  const title = input.title.trim();
  const projectUrl = input.projectUrl?.trim() || null;
  const projectText = input.projectText?.trim() || null;
  if (!title) fail("Informe o nome do projeto.");
  if (!projectUrl && !projectText) fail("Envie um link ou descreva o projeto para avaliação.");
  const { data: submission, error } = await db.from("training_project_submissions").insert({
    training_id: training.id,
    enrollment_id: currentEnrollment.id,
    customer_id: customer.id,
    review_credit_id: credit.id,
    submission_number: submissionNumber,
    title,
    project_url: projectUrl,
    project_text: projectText,
    status: "submitted",
  }).select("id,submission_number,status,submitted_at").single();
  if (error || !submission) fail("Não foi possível enviar o projeto.");
  await db.from("training_project_review_credits").update({ status: "used", used_at: new Date().toISOString() }).eq("id", credit.id);
...
export async function saveDoMamaoTrainingState(userId: string, email: string | null, state: StateRecord) {
  ...
  const eligibleDay = started + Number(training.minimum_days ?? DEFAULT_MINIMUM_DAYS) * 86400000;
  ...
  if (!completedAt && progress >= 100 && Date.now() >= eligibleDay) {
    ...
    certificateAvailableAt = new Date(Date.now() + 15 * 86400000).toISOString();
  }
  const currentPanel = typeof state.lastPanel === "string" ? state.lastPanel.slice(0, 80) : "inicio";
  const { error } = await db.from("training_state").upsert({
    training_id: training.id,
    customer_id: customer.id,
    state,
