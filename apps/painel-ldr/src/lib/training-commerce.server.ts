...
async function paidOrder(customerId: string) {
  const { data, error } = await db
    .from("orders")
    .select(
      "id,catalog_key,payment_status,amount_cents,currency,stripe_checkout_session_id,metadata,created_at",
    )
...
async function enrollment(customerId: string, trainingId: string) {
  const { data, error } = await db
    ...
    .select(
      "id,training_id,customer_id,active,enrolled_at,cohort_id,progress_percent,completed_at,certificate_available_at,training_cohorts(cohort_number,capacity,status)",
    )
...
async function ensureOwnerEnrollment(customerId: string, trainingId: string, email: string | null) {
  ...
  const { error } = await db
    .from("training_enrollments")
    .upsert(
      { training_id: trainingId, customer_id: customerId, active: true },
...
async function provisionFromPaidOrder(customerId: string, order: any) {
  ...
  const { error } = await db.rpc("assign_do_mamao_training_enrollment", {
    ...
    _payment_intent_id:
      typeof metadata.stripe_payment_intent_id === "string"
  ...
  if (error)
    fail("Pagamento confirmado, mas não foi possível concluir sua matrícula automaticamente.");
}
...
export async function getDoMamaoTrainingOffer(userId: string, email: string | null) {
  ...
  return {
    ...
    description:
      ...
      "Treinamento empreendedor de 3 meses e 300 horas com Sistema S8, atividades, quizzes, projeto, fórum e encontros ao vivo.",
...
export async function createDoMamaoTrainingCheckout(
  userId: string,
  email: string | null,
  market: Market,
) {
  ...
  params.set(
    "success_url",
    `${appOrigin}/cliente/biblioteca?payment=success&product=training&session_id={CHECKOUT_SESSION_ID}`,
  );
...
export async function getDoMamaoTrainingExperience(userId: string, email: string | null) {
  ...
  const { data: cloud } = await db
    .from("training_state")
    .select("state,progress_percent,current_panel,started_at,completed_at,certificate_available_at")
  ...
  return {
    ...
    certificateAvailableAt:
      cloud?.certificate_available_at ?? currentEnrollment.certificate_available_at ?? null,
  };
}
...
function countTrainingProgress(state: StateRecord) {
  ...
  const quiz = (state.quiz && typeof state.quiz === "object" ? state.quiz : {}) as Record<
    string,
    unknown
  >;
  const reflections = (
    state.reflections && typeof state.reflections === "object" ? state.reflections : {}
  ) as Record<string, unknown>;
  let written = 0;
  for (const value of Object.values(reflections))
    if (typeof value === "string" && value.trim().length >= 40) written++;
  const done =
    Math.min(60, Object.keys(answers).length) +
    Math.min(36, Object.keys(quiz).length) +
    Math.min(30, written);
  return Math.max(0, Math.min(100, Math.round((done / 126) * 100)));
}

export async function saveDoMamaoTrainingState(
  userId: string,
  email: string | null,
  state: StateRecord,
) {
  ...
  if (serialized.length > 700_000)
    fail("Dados do treinamento excederam o limite de sincronização.");
  ...
  const currentPanel =
    typeof state.lastPanel === "string" ? state.lastPanel.slice(0, 80) : "inicio";
  const { error } = await db
    .from("training_state")
    .upsert(
      {
  ...
  await db
    ...
    .upsert(
      {
        customer_id: customer.id,
        product_key: TRAINING_SLUG,
        progress_percent: progress,
        current_location: currentPanel,
        updated_at: new Date().toISOString(),
      },
