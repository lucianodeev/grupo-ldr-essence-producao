...
export async function notificationsFor(userId:string){const cid=await customerId(userId);if(!cid)return [];const {data}=await db.from("notification_outbox").select("id,subject,body,status,metadata,created_at").eq("audience_type","client").eq("target_id",cid).eq("channel","in_app").contains("metadata",{source:"academic_network"}).order("created_at",{ascending:false}).limit(100);const rows=data??[];if(!rows.length)return rows;const {data:reads}=await db.from("academic_notification_reads").select("notification_id,read_at").eq("user_id",userId).in("notification_id",rows.map((x:any)=>x.id));const readMap=new Map((reads??[]).map((x:any)=>[x.notification_id,x.read_at]));return rows.map((x:any)=>({...x,readAt:readMap.get(x.id)??null,isRead:readMap.has(x.id)}))}
