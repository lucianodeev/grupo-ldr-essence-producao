...
export async function editorialSnapshot(userId:string,input?:{limit?:number;country?:string|null;followingOnly?:boolean}){
  try{
    ...
    const [{data:settings},{count:real24},{data:real7Rows},{data:follows},{data:saved},{data:reacted}]=await Promise.all([
      ...
      db.from("academic_saved_editorial_posts").select("post_id").eq("user_id",userId),
    ...
    if(input?.followingOnly&&followedIds.size)q=q.in("profile_id",[...followedIds]);
    else if(input?.followingOnly&&!followedIds.size)return {posts:[],profiles:profiles??[],articles:[],suggestions:(profiles??[]).slice(0,6),stats:{editorialPercent,real24:real24??0,real7,activeUsers},settings,followedIds:[...followedIds]};
    if(input?.country&&profiles?.length)q=q.in("profile_id",profiles.map((p:any)=>p.id));
    const {data:rawPosts}=await q;
    const target=Math.min(limit,Math.max(2,Math.ceil(limit*editorialPercent/100)));
    const picked:any[]=[];let last:string|null=null;for(const p of rawPosts??[]){if(!profileMap.has(p.profile_id)||p.profile_id===last)continue;picked.push(p);last=p.profile_id;if(picked.length>=target)break;}
...
export async function toggleEditorialSupport(userId:string,postId:string){const {data}=await db.from("academic_editorial_reactions").select("post_id").eq("post_id",postId).eq("user_id",userId).maybeSingle();if(data){await db.from("academic_editorial_reactions").delete().eq("post_id",postId).eq("user_id",userId);return {supported:false}}const {error}=await db.from("academic_editorial_reactions").insert({post_id:postId,user_id:userId,reaction_type:"support"});if(error)fail("Não foi possível acolher esta publicação.");return {supported:true}}
export async function toggleEditorialSave(userId:string,postId:string){const {data}=await db.from("academic_saved_editorial_posts").select("post_id").eq("post_id",postId).eq("user_id",userId).maybeSingle();if(data){await db.from("academic_saved_editorial_posts").delete().eq("post_id",postId).eq("user_id",userId);return {saved:false}}const {error}=await db.from("academic_saved_editorial_posts").insert({post_id:postId,user_id:userId});if(error)fail("Não foi possível salvar esta publicação.");return {saved:true}}
