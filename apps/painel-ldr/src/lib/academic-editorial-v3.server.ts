...
export async function editorialSnapshot(userId:string,input?:{limit?:number;country?:string|null;followingOnly?:boolean;savedOnly?:boolean}){
  try{
    ...
    const [{data:editorComments},{data:userComments},{data:reactionRows},{data:articles}]=await Promise.all([
      ...
      ids.length?db.from("academic_editorial_user_comments").select("id,post_id,user_id,body,parent_comment_id,created_at,updated_at").in("post_id",ids).eq("status","active").order("created_at"):{data:[]},
    ...
    for(const c of editorComments??[]){const a=commentsBy.get(c.post_id)??[];a.push({...c,parent_comment_id:null,own:false,editorial:true,author:{...profileMap.get(c.profile_id),name:profileMap.get(c.profile_id)?.display_name}});commentsBy.set(c.post_id,a)}
    for(const c of userComments??[]){const a=commentsBy.get(c.post_id)??[];a.push({...c,own:c.user_id===userId,editorial:false,author:{name:nameMap.get(c.user_id)||"Membro LDR"}});commentsBy.set(c.post_id,a)}
...
export async function addEditorialUserComment(userId:string,postId:string,bodyRaw:string,parentCommentId?:string|null){const body=clean(bodyRaw,4000);if(!body)fail("Escreva um comentário.");let parent_comment_id:string|null=null;if(parentCommentId){const {data:parent,error:parentError}=await db.from("academic_editorial_user_comments").select("id,post_id,status").eq("id",parentCommentId).maybeSingle();if(parentError||!parent||parent.status!=="active"||parent.post_id!==postId)fail("Comentário de resposta inválido.");parent_comment_id=parent.id}const since=new Date(Date.now()-15000).toISOString();const {count}=await db.from("academic_editorial_user_comments").select("id",{count:"exact",head:true}).eq("user_id",userId).gte("created_at",since);if((count??0)>0)fail("Aguarde alguns segundos antes de comentar novamente.");const {data,error}=await db.from("academic_editorial_user_comments").insert({post_id:postId,user_id:userId,body,parent_comment_id}).select("id").single();if(error)fail("Não foi possível comentar.");return data}
export async function updateEditorialUserComment(userId:string,commentId:string,bodyRaw:string){const body=clean(bodyRaw,4000);if(!body)fail("Escreva um comentário.");const {data,error}=await db.from("academic_editorial_user_comments").update({body,updated_at:new Date().toISOString()}).eq("id",commentId).eq("user_id",userId).eq("status","active").select("id").maybeSingle();if(error||!data)fail("Não foi possível editar este comentário.");return data}
export async function deleteEditorialUserComment(userId:string,commentId:string){const {data,error}=await db.from("academic_editorial_user_comments").update({status:"hidden",updated_at:new Date().toISOString()}).eq("id",commentId).eq("user_id",userId).eq("status","active").select("id").maybeSingle();if(error||!data)fail("Não foi possível excluir este comentário.");return data}
