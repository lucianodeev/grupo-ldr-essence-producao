...
export const refreshCareerIntelligence=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).handler(async({context})=>{
 ...
 const goal=(goals??[])[0]; const targetTitle=String(goal?.target_title??"").trim(); const normalizedTargetTitle=targetTitle.toLowerCase(); const skills=[...new Set((proofs??[]).map((p:any)=>p.competency_key).filter(Boolean))];
 ...
 const actions:any[]=[]; if(goal)actions.push({user_id:uid,context_type:"career",context_reference:goal.id,action_type:"build_portfolio",title:targetTitle?"Fortaleça seu portfólio para "+targetTitle:"Fortaleça seu portfólio profissional",rationale:{summary:"Objetivo ativo no Career GPS conectado às evidências disponíveis."}});
