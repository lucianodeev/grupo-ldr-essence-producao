...
export const refreshCareerIntelligence=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).handler(async({context})=>{
 ...
 await db.from("ldr_copilot_actions").delete().eq("user_id",uid).eq("status","suggested").in("context_type",["career","portfolio","project"]); if(actions.length)await db.from("ldr_copilot_actions").insert(actions);
