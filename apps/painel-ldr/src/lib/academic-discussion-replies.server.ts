...
async function requireAcademicAccess(userId:string){
  const [{data:customer},{data:trial}]=await Promise.all([
    db.from("customers").select("id").eq("auth_user_id",userId).maybeSingle(),
    db.from("academic_access_trials").select("expires_at,status").eq("user_id",userId).maybeSingle()
  ]);
  ...
  if(customer?.id){
    ...
    subscription=["active","trialing"].includes(String(sub?.status??""));
  }
  const trialActive=trial?.status==="active"&&new Date(trial.expires_at).getTime()>Date.now();
  if(!subscription&&!trialActive){
    // Preserve current server behavior for existing profiles/admin flows by allowing
    // the canonical academic server to remain the authority for root comments.
    // Threaded replies require an existing academic profile and an active entitlement.
    const {data:profile}=await db.from("academic_profiles").select("id").eq("user_id",userId).maybeSingle();
    if(!profile)fail("Este recurso faz parte da assinatura da LDR Essence Academy.");
  }
}
