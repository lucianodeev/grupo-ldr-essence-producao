...
export async function verifyOwnedFile(userId:string,path:string,purpose:Purpose){if(!path.startsWith(`${userId}/${purpose}/`))fail("Arquivo inválido.");const {data,error}=await db.storage.from(BUCKET).download(path);if(error||!data)fail("Arquivo não encontrado.");const max=purpose==="posts"?POST_MAX:IMAGE_MAX;if(data.size<=0||data.size>max)fail(`O arquivo deve ter no máximo ${max/1024/1024} MB.`);const bytes=new Uint8Array(await data.arrayBuffer());const real=mimeFromBytes(bytes.slice(0,16));const allowed=purpose==="posts"?POST_ALLOWED:IMAGE_ALLOWED;if(!real||!allowed.has(real))fail("Formato de arquivo inválido.");return {mime:real,size:data.size}}
async function ensureAcademicProfile(userId:string){const {data:existing,error}=await db.from("academic_profiles").select("id").eq("user_id",userId).maybeSingle();if(error)fail("Não foi possível carregar o perfil acadêmico.");if(existing)return existing;const created=await db.from("academic_profiles").upsert({user_id:userId},{onConflict:"user_id",ignoreDuplicates:true});if(created.error)fail("Não foi possível criar o perfil acadêmico.");const {data,error:readError}=await db.from("academic_profiles").select("id").eq("user_id",userId).single();if(readError||!data)fail("Não foi possível carregar o perfil acadêmico.");return data}
...
export async function finalizeAcademicPostMedia(userId:string,input:{postId:string;path:string;altText?:string;fileName?:string},pending=false){const verified=await verifyOwnedFile(userId,input.path,"posts");const {data:post}=await db.from("academic_posts").select("id").eq("id",input.postId).eq("user_id",userId).eq("status",pending?"hidden":"active").maybeSingle();if(!post)fail("Publicação não encontrada.");const {data,error}=await db.from("academic_post_media").insert({post_id:input.postId,owner_user_id:userId,storage_path:input.path,mime_type:verified.mime,alt_text:String(input.altText??"").trim().slice(0,300),file_name:safeName(input.fileName||input.path.split("/").pop()||"arquivo"),sort_order:0}).select("id").single();if(error)fail("Não foi possível vincular o arquivo à publicação.");return data}
