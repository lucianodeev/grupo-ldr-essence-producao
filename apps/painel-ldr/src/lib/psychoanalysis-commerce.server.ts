...
async function ensureProgram(){
  ...
  const {data,error}=await db.from("training_programs").insert({slug:PRODUCT_SLUG,title:TITLE,description:"Formação livre em Psicanálise com 12 módulos, 180 unidades de aprendizagem, carga horária formativa total de 1.200 horas e 6 encontros ao vivo.",status:"published"}).select("id,slug,title,status").single();
