...
const TOTAL_LESSONS=200;
const TOTAL_HOURS=1200;
const MINIMUM_DAYS=200;
...
async function ensureProgram(){
  ...
  if(existing){
    if(existing.status!=="published"){
      const {data:published}=await db.from("training_programs").update({status:"published",description:"Formação livre em Psicanálise com 13 módulos, 200 unidades de aprendizagem, carga horária formativa total de 1.200 horas e 6 encontros ao vivo."}).eq("id",existing.id).select("id,slug,title,status").single();
      return published??{...existing,status:"published"};
    }
    return existing;
  }
  const {data,error}=await db.from("training_programs").insert({slug:PRODUCT_SLUG,title:TITLE,description:"Formação livre em Psicanálise com 13 módulos, 200 unidades de aprendizagem, carga horária formativa total de 1.200 horas e 6 encontros ao vivo.",status:"published"}).select("id,slug,title,status").single();
...
async function assignCohortBestEffort(trainingId:string,enrollmentId:string){
  ...
  }catch(error){console.warn("psychoanalysis cohort assignment skipped",error);}
}
...
async function ensureEnrollment(customerId:string,entitled:boolean){
  ...
  try{
    ...
    return enrollment;
  }catch(error){console.warn("psychoanalysis enrollment provisioning skipped",error);return null;}
}
...
export async function getPsychoanalysisOffer(userId:string,email:string|null){
