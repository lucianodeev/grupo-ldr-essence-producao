...
export async function createClientEntrepreneurComboCheckout(userId:string,email:string|null,market:DigitalMarket){
  ...
  const {getPublicDoMamaoTrainingOffer}=await import("@/lib/training-commerce.server"); const trainingOffer=await getPublicDoMamaoTrainingOffer(); const amountCents=market==="BR"?trainingOffer.priceBrlCents:trainingOffer.priceEurCents; const currency=market==="BR"?"BRL":"EUR";
