...
const PRODUCT_KEY="orientacao_psicanalitica_escrita";
const TITLE="Orientação Psicanalítica Escrita";
const OWNER_EMAIL="llucianouam@gmail.com";
type Market="BR"|"INTL";
type PackageMinutes=10|30|60;

const PROMO_PRICES={
  INTL:{10:90,30:100,60:190},
  BR:{10:535,30:595,60:1130},
} as const;
...
async function promoFor(customerId:string){
  const {data}=await db.from("psychoanalytic_written_promos").select("first_purchase_at,promo_expires_at").eq("customer_id",customerId).maybeSingle();
...
export async function getWrittenOrientationOffer(userId:string,email:string|null){
  ...
  const {data:sessions}=await db.from("psychoanalytic_written_sessions").select("id,package_minutes,remaining_seconds,currency,amount_cents,promo_applied,status,started_at,ended_at,created_at,updated_at").eq("customer_id",customer.id).order("created_at",{ascending:false}).limit(30);
...
export async function createWrittenOrientationCheckout(userId:string,email:string|null,market:Market,packageMinutes:PackageMinutes){
  ...
  params.set("success_url",`${appOrigin()}/cliente/orientacao-psicanalitica?payment=success&session_id={CHECKOUT_SESSION_ID}`);params.set("cancel_url",`${appOrigin()}/cliente/orientacao-psicanalitica?payment=cancel`);params.set("client_reference_id",userId);
...
export async function activateWrittenOrientationCheckout(userId:string,email:string|null,checkoutSessionId:string){
  if(!checkoutSessionId)fail("Sessão de pagamento inválida.");
  const customer=await customerFor(userId,email);
  const {data:existing}=await db.from("psychoanalytic_written_sessions").select("id").eq("stripe_checkout_session_id",checkoutSessionId).maybeSingle();if(existing?.id)return {sessionId:existing.id};
  ...
  if(!promo.started){const now=new Date();const expires=new Date(now.getTime()+30*86_400_000);await db.from("psychoanalytic_written_promos").insert({customer_id:customer.id,first_purchase_at:now.toISOString(),promo_expires_at:expires.toISOString()});promo=await promoFor(customer.id);}
  const {data:created,error}=await db.from("psychoanalytic_written_sessions").insert({customer_id:customer.id,auth_user_id:userId,order_id:orderId,stripe_checkout_session_id:checkoutSessionId,package_minutes:minutes,purchased_seconds:minutes*60,remaining_seconds:minutes*60,currency,amount_cents:amountCents,promo_applied:promoApplied,status:"paid"}).select("id").single();
  if(error||!created)fail("Não foi possível criar a orientação.");return {sessionId:created.id,promo};
}

async function ownedSession(userId:string,email:string|null,sessionId:string){const customer=await customerFor(userId,email);const {data,error}=await db.from("psychoanalytic_written_sessions").select("*").eq("id",sessionId).eq("customer_id",customer.id).maybeSingle();if(error||!data)fail("Orientação não encontrada.");return {customer,session:data};}
...
async function normalizeSession(session:any){const t=consumeTimer(session);if(t.elapsed>0){const closed=t.remaining<=0;const patch:any={remaining_seconds:t.remaining,last_timer_started_at:closed?null:new Date().toISOString(),timer_running:!closed,updated_at:new Date().toISOString()};if(closed){patch.status="closed";patch.ended_at=new Date().toISOString();}await db.from("psychoanalytic_written_sessions").update(patch).eq("id",session.id);return {...session,...patch};}return session;}

export async function getWrittenOrientationSession(userId:string,email:string|null,sessionId:string){const {session}=await ownedSession(userId,email,sessionId);const normalized=await normalizeSession(session);const {data:messages}=await db.from("psychoanalytic_written_messages").select("id,sender_type,message,created_at,read_at").eq("session_id",sessionId).order("created_at",{ascending:true});return {session:normalized,messages:messages??[]};}

async function notifyMaster(customer:any,sessionId:string,message:string){try{const {data:profile}=await db.from("profiles").select("id").ilike("email",OWNER_EMAIL).maybeSingle();if(!profile?.id)return;await db.from("notification_outbox").insert({audience_type:"professional",target_id:profile.id,channel:"in_app",event_type:"manual",subject:"Nova Orientação Psicanalítica Escrita",body:`${customer.fullName??customer.full_name??customer.email??"Cliente"}: ${message.slice(0,180)}`,scheduled_for:new Date().toISOString(),status:"sent",sent_at:new Date().toISOString()});}catch(error){console.warn("written orientation notification skipped",error);}}

export async function sendWrittenOrientationClientMessage(userId:string,email:string|null,sessionId:string,message:string,consent:boolean){
  ...
  await db.from("psychoanalytic_written_sessions").update(patch).eq("id",sessionId);const {error}=await db.from("psychoanalytic_written_messages").insert({session_id:sessionId,sender_type:"client",sender_user_id:userId,message:body});if(error)throw error;await notifyMaster(customer,sessionId,body);return {ok:true};
}

export async function getWrittenOrientationAdmin(supabase:any,userId:string){await requireMaster(supabase,userId);const {data:sessions,error}=await db.from("psychoanalytic_written_sessions").select("id,customer_id,package_minutes,remaining_seconds,currency,amount_cents,promo_applied,status,started_at,last_client_message_at,last_professional_message_at,created_at,updated_at,customers(full_name,email)").order("updated_at",{ascending:false}).limit(100);if(error)throw error;return sessions??[];}
export async function getWrittenOrientationAdminSession(supabase:any,userId:string,sessionId:string){await requireMaster(supabase,userId);const {data:s,error}=await db.from("psychoanalytic_written_sessions").select("*,customers(full_name,email)").eq("id",sessionId).maybeSingle();if(error||!s)fail("Orientação não encontrada.");const session=await normalizeSession(s);const {data:messages}=await db.from("psychoanalytic_written_messages").select("id,sender_type,message,created_at,read_at").eq("session_id",sessionId).order("created_at",{ascending:true});return {session,messages:messages??[]};}
export async function sendWrittenOrientationProfessionalMessage(supabase:any,userId:string,sessionId:string,message:string){await requireMaster(supabase,userId);const body=message.trim();if(!body||body.length>5000)fail("Mensagem inválida.");const {data:raw}=await db.from("psychoanalytic_written_sessions").select("*").eq("id",sessionId).maybeSingle();if(!raw)fail("Orientação não encontrada.");const session=await normalizeSession(raw);if(session.status==="closed"||Number(session.remaining_seconds)<=0)fail("Esta orientação foi encerrada.");const now=new Date().toISOString();const {error}=await db.from("psychoanalytic_written_messages").insert({session_id:sessionId,sender_type:"professional",sender_user_id:userId,message:body});if(error)throw error;await db.from("psychoanalytic_written_sessions").update({status:"waiting_client",timer_running:true,last_timer_started_at:now,last_professional_message_at:now,updated_at:now}).eq("id",sessionId);return {ok:true};}
