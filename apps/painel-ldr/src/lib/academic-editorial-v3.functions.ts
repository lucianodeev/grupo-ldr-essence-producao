...
export const academicEditorialSnapshot=createServerFn({method:"GET"}).middleware(auth).inputValidator((data?:{limit?:number;country?:string|null;followingOnly?:boolean})=>data??{}).handler(async({context,data})=>(await import("@/lib/academic-editorial-v3.server")).editorialSnapshot(context.userId,data));
