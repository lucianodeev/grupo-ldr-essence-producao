...
export const academicUpdateEditorialComment=createServerFn({method:"POST"}).middleware(auth).inputValidator((data:{id:string;body:string})=>data).handler(async({context,data})=>(await import("@/lib/academic-editorial-v3.server")).updateEditorialUserComment(context.userId,data.id,data.body));
export const academicDeleteEditorialComment=createServerFn({method:"POST"}).middleware(auth).inputValidator((data:{id:string})=>data).handler(async({context,data})=>(await import("@/lib/academic-editorial-v3.server")).deleteEditorialUserComment(context.userId,data.id));
