...
export async function resolveAccess(supabase: Client, userId: string) {
  // The authenticated user's own profile and role are intentionally readable
  // through RLS. Using the request-scoped client keeps login/access validation
  // functional on hosts that do not carry the service-role secret.
  const [
