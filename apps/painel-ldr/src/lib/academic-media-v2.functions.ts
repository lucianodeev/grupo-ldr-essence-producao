...
export const academicFinalizePostImage=createServerFn({method:"POST"}).middleware(auth).inputValidator((data:{postId:string;path:string;altText?:string})=>data).handler(async({context,data})=>(await import("@/lib/academic-media-v2.server")).finalizeAcademicPostImage(context.userId,data));
