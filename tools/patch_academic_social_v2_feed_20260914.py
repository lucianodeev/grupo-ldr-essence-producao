...
needle='  const {data,isLoading,error}=useQuery({queryKey:["academic-network",communityId,search,tab],queryFn:()=>snapshotFn({data:{communityId:communityId||null,search:search||null,savedOnly:tab==="saved"}})});'
...
seg=seg.replace('data.posts?.length?data.posts.map((p:any)=>','visiblePosts.length?visiblePosts.map((p:any)=>',1)
u=u[:start]+seg+u[end if end>start else len(u):]
marker='<div><b className="text-sm">{t.highlight}</b><p className="mt-1 text-xs text-muted-foreground">{t.highlightSub}</p></div>'
...
load='''{feedMode!=="articles"&&data.hasMore&&<div className="pt-2 text-center"><button disabled={more.isPending} onClick={()=>more.mutate((data.posts?.length??0)+extraPosts.length)} className="min-h-11 rounded-xl border bg-card px-5 text-xs font-black">{more.isPending?"CARREGANDO…":"CARREGAR MAIS"}</button></div>}'''
start=u.find('function Feed('); end=u.find('\nfunction Communities',start)
if end>start and 'CARREGAR MAIS' not in u[start:end]:
    seg=u[start:end]; pos=seg.rfind('</main>')
